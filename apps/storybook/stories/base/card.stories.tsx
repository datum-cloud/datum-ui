    ...
    docs: {
      description: {
        component:
          ...
          + '`CardAction`, `CardContent`, `CardField`, and `CardFooter`.\n\n'
...
export const SectionedWithFooter: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Settings-style card: bordered header, `CardField` definition rows (two equal columns), '
          + 'and a `CardSaveBar` footer. The bar owns the status line and Cancel / Save buttons; '
          + 'Save stays disabled until `changeCount` is non-zero and `errorCount` is zero.',
      },
    },
  },
  render: () => (
    <Card size="sm" sectioned className="w-[520px]">
      <CardHeader size="sm" bordered>
        <CardTitle className="text-sm">TLS &amp; Certificates</CardTitle>
        <CardAction>
      ...
      <CardSaveBar changeCount={2} onCancel={() => {}} onSave={() => {}} />
    </Card>
  ),
}
...
export const SaveBarStates: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '`CardSaveBar` across its states: clean, dirty, blocked by validation errors, and saving.',
      },
    },
  },
  render: () => (
    <div className="flex w-[520px] flex-col gap-4">
      <Card size="sm" sectioned>
        <CardSaveBar changeCount={0} onCancel={() => {}} onSave={() => {}} />
      </Card>
      <Card size="sm" sectioned>
        <CardSaveBar changeCount={1} onCancel={() => {}} onSave={() => {}} />
      </Card>
      <Card size="sm" sectioned>
        <CardSaveBar changeCount={2} errorCount={1} onCancel={() => {}} onSave={() => {}} />
      </Card>
      <Card size="sm" sectioned>
        <CardSaveBar changeCount={1} saving onCancel={() => {}} onSave={() => {}} />
      </Card>
    </div>
  ),
}
