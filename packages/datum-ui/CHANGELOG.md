...
- ef955a1: Tidier `AppNavigation` sections and spacing.
  
  - `group` headers line up with the item icons below them, get space above, and hide in the icon rail, which now shows each item's icon instead of one button per section.
  - Items under a `group` render as top-level rows, so they match the height of other top-level rows.
  - Rows sit further from the sidebar edge, and nested rows line up under their parent's label.

## 3.1.0

### Minor Changes

- f33a169: Headings below 16px now have somewhere to sit. The type scale's display steps start at 16px (`Title` level 6), so a section or card heading at 14px had no level and had to fall back to a raw size class.
...
- 57eddf2: One type scale, `5xs` through `8xl`, now backs both the raw `text-*` utilities and the Typography components, so `text-sm` and `<Text size="sm">` always render the same size. The scale is defined in the alpha theme; Storybook "Docs/Type Scale" has the rules and the full table.
...
- 25e0bc6: `DataTable.Content`'s `density="compact"` cell padding moves from `py-0.5` to `py-2` (~36-40px rows), matching a same-day follow-up fix to staff-portal's own list-table row height. `DataTable.ListPanel` now forwards `tableClassName`, `headerClassName`, `headerRowClassName`, `headerCellClassName`, `bodyClassName`, `rowClassName`, and `cellClassName` to its internal `DataTable.Content`, so an app-specific cell override (sizing a row-actions menu, a copy button rendered inside a cell) can actually reach the table — previously there was no way to apply one through `ListPanel`.

## 2.11.0

### Minor Changes

- 57d3553: `DataTable` gains a compact "list table" surface so a plugin repo with no Tailwind build of its own (e.g. compute) can build a list page that looks like the rest of the product, and staff-portal's own `ListTable` can be refactored to consume the shared piece instead of duplicating it.
...
- 0507fe3: Assistant chat history can archive chats and confirm deletes. All new props are optional; hosts that pass none of them see no change.
...
- 27f6760: Fix SettingsNavItem `asChild` so a router Link can own the element, and size SettingsNav items at `text-xs` (13px) to match the app nav. `cva` stays private; the danger icon reads `data-variant`. CardSaveBar's live region wraps only the status text so Cancel/Save are not re-announced.
...
- a354fef: Switch now shows its state as a glyph: a cross when off, a check when on, drawn inside the thumb. Previously on and off were carried only by the thumb's position and the track's colour, which is easy to misread at a glance — particularly on a dense settings page where several switches sit in a column.
...
- b6e10b6: Bring the logs explorer closer to a Vercel-style log view: column sizing, custom columns, accurate loading, relative time presets, a timeline histogram, and a tighter detail header.
  ...
  Empty and error states render as centred in-table messages instead of a bare red line: the error keeps `role="alert"`, shows the raw message in a code chip, and offers Retry when `onRefresh` is set; the empty state offers Clear filters when a filter or search is active. Only one of loading, error, or empty renders at a time, and a failed refresh with rows already on screen keeps the rows and shows a slim banner above them instead of replacing them. Unknown column ids are skipped with a development warning rather than crashing the table.
  
  HTTP method, path, and status also fall back to Envoy/OTEL stream labels when the log Body is empty. `logRequestHost` picks the Host cell: an explicit `host` label, then `requested_server_name` / `x_forwarded_host`, then `authority`, then `resource_name`. Client-side search (`filterEntries`, `searchableLogText`) matches the line and those displayed access-log fields only, not every label. Fixtures include a sanitised staging ALB `query_range` (`albQueryRangeFixture`).
